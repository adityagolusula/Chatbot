...
function SingleState(input){
    this.input = input;
    this.answer = '';
    this.next = false;
    return this;
};
SingleState.prototype.hasNext = function(){
    return this.next;
};
function State(wrapper, SingleState, form, params, originalFormHtml) {
    this.form = form;
    this.wrapper = wrapper;
    this.current = SingleState;
    this.answers = {};
    this.parameters = params;
    this.originalFormHtml = originalFormHtml;
    this.scrollDown = function() {
         $(this.wrapper).find("#messages").stop(true).animate({scrollTop: $(this.wrapper).find('#messages')[0].scrollHeight}, 600);
    }.bind(this);
};
...
State.prototype.newState = function(options) {
...
State.prototype.printQuestion = function(){
...
State.prototype.printAnswers = function(answers, multiple){
    var opened = false;
    ...
    if(this.parameters.selectInputStyle=='disable') {
        ...
        if(!this.current.input.multiple) {
            ...
            $(this.wrapper).find('#conv button').css({display: 'none'});
        } else {
            $(this.wrapper).find('#'+this.parameters.inputIdName).prop('disabled', true);
            $(this.wrapper).find('#'+this.parameters.inputIdName).attr('placeholder', this.parameters.selectInputDisabledText);
        }
...
State.prototype.answerWith = function(answerText, answerObject) {
    ...
    if(this.current.input.type=='select' && this.parameters.selectInputStyle=='disable') {
        ...
        if(!this.current.input.multiple) {
            ...
            $(this.wrapper).find('#conv button').css({display: 'block'});
        } else {
            $(this.wrapper).find('#'+this.parameters.inputIdName).prop('disabled', false);
            $(this.wrapper).find('#'+this.parameters.inputIdName).attr('placeholder', this.parameters.placeHolder);
        }
...
(function($){
    $.fn.conv = function(options){
        ...
        var parameters = $.extend(true, {}, {
            ...
            eventList : {
                onSubmitForm : function(State) {
                    console.log('completed');
                    State.form.submit();
                    return true;
                },
                onInputSubmit : function(State, readyCallback) {
                    if(State.current.input.hasOwnProperty('callback')) {
                        if(typeof State.current.input.callback === 'string') {
                            window[State.current.input.callback](State, readyCallback);
                        } else {
                            State.current.input.callback(State, readyCallback);
                        }
                    } else {
                        readyCallback();
                    }
        ...
        */
        ...
        if(inputs.length) {
            ...
            switch(parameters.typeInputUi) {
                case 'input':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convDynamic"><div class="options dragscroll"></div><input id="' + parameters.inputIdName + '" type="text" placeholder="'+ parameters.placeHolder +'" class="userInputDynamic"></><button type="submit" class="submit">'+parameters.buttonText+'</button><span class="clear"></span></form>');
                    break;
                case 'textarea':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convDynamic"><div class="options dragscroll"></div><textarea id="' + parameters.inputIdName + '" rows="1" placeholder="'+ parameters.placeHolder +'" class="userInputDynamic"></textarea><button type="submit" class="submit">'+parameters.buttonText+'</button><span class="clear"></span></form>');
                    break;
                default :
